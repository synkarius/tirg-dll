...
namespace trg {
	...
	void calc_lums(const std::vector<std::vector<Rgb> > &raw) {
		...
		while (y1 < h) {
			...
			while (x1 < w) {
				int sm = 0;
				for (y = y1; y < minimum(y2, h); ++y) {
					for (x = x1; x < minimum(x2, w); ++x) {
						Rgb px = raw[y][x];
				...
				double sr = sm * 1.0 / (minimum(y2, h) - y1) / (minimum(x2, w) - x1);
				double cc = 0;
				for (y = y1; y < minimum(y2, h); ++y)
					for (x = x1; x < minimum(x2, w); ++x)
						cc += fabs(sr - lum[y][x]);
				cc = cc / (minimum(y2, h) - y1) / (minimum(x2, w) - x1);
				for (y = y1; y < minimum(y2, h); ++y)
					for (x = x1; x < minimum(x2, w); ++x)
						cyx[y][x] = (UC)maximum(35.0, cc);
				x1 = x2;
				x2 = x1 + zw;
			}
			y1 = y2;
	...
	int stroke_calc(int p1, int q1) {
		int p2 = minimum(h - 1, p1 + dh);
		int q2 = minimum(w - 1, q1 + dw);
		int nm = (p2 - p1) * (q2 - q1);
		double u = 0.0;
		std::vector<int> x(10);
		for (int i = p1; i < p2; ++i) {
